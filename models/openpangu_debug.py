...
SAVE_PATH="/work/jijun/code/radlads/test_tensors"
def debug_tensor(t, name:str):
    if t is None:
        return
    target = torch.load(f"{SAVE_PATH}/{name}.pt", map_location=t.device)
    assert target.shape == t.shape, f"{name=} {target.shape=} != {t.shape=}"
    assert torch.allclose(t, target), f"{name=} target mean={target.mean().item()} std={target.std().item()} != current tensor mean={t.mean().item()} std={t.std().item()}"
    print(f"{name=} ok")
...
def generate_rotary_embedding(max_seqlen: int, dim: int, rotary_percent: float = 1.0, theta: float = 10000.0, scale: float = 1):
    ...
    emb = torch.cat((angles, angles), dim=-1)
    # print(f"[generate_rotary_embedding] {rotary_percent=}, {dim=} {angles.shape=} {emb.shape=}")
...
class TMix_openpangu(nn.Module):
    ...
    def __init__(self, config, layer_id):
        ...
        # ! is_causal is needed to use flash attention
        self.is_causal = True
        ...
        # print(f"[__init__] {layer_id=} {attention_bias=} {self.param_sink_number=} {self.param_sink_with_value=} {self.attn_groupnorm=} {self.attn_elementwise_gate=}")
    ...
    def forward(self, x, reset_mask, v_first, last_model_state:ModelState, shared:Shared, output_attentions:bool=False, past_key_values:Cache=None, **kwargs):
        if last_model_state is not None:
            ...
            tmix_state = TimeMixState(wkv_state, last_state.shift_state)
        else:
            tmix_state = None
        ...
        # debug_tensor(x, f"layer_{self.layer_id}.hidden_states")
        ...
        # debug_tensor(q, f"layer_{self.layer_id}.query_states")
        # debug_tensor(k, f"layer_{self.layer_id}.key_states")
        # debug_tensor(v, f"layer_{self.layer_id}.value_states")
        # debug_tensor(gate_score, f"layer_{self.layer_id}.gate_score")
        ...
        # debug_tensor(q, f"layer_{self.layer_id}.query_states_rope")
        # debug_tensor(k, f"layer_{self.layer_id}.key_states_rope")
...
ƒ
        ...
        # debug_tensor(y, f"layer_{self.layer_id}.attn_output")
        if self.attn_groupnorm:
            y = self.groupnorm(y)
        ...
        y = self.o_proj(y)
        # debug_tensor(y, f"layer_{self.layer_id}.attn_output_o_proj")
...
class TMix_openpangurwkv6(TMix_openpangu):
    ...
    def forward(self, x, reset_mask, v_first, last_model_state:ModelState, shared:Shared, output_attentions:bool=False, **kwargs):
        last_state = last_model_state.block_states[self.layer_id].time_mix_state
...
class TMix_openpangurwkv7(TMix_openpangu):
    ...
    def forward(self, x, reset_mask, v_first, last_model_state:ModelState, shared:Shared, output_attentions:bool=False, **kwargs):
        ...
        if self.config.use_qk_rmsnorm:
            # if self.layer_id == 1:
                # print (f"From TMix_qwen3rwkv7 forward: {self.config.use_qk_rmsnorm=}")
            r = self.q_norm(r)
            k = self.k_norm(k)
        if self.config.use_pos_emb:
...
class TMix_openpangugdn_base(TMix_openpangu):
    ...
    def __init__(self, config:Transformer_Config, layer_id):
        ...
        ### from https://github.com/fla-org/flash-linear-attention/blob/main/fla/layers/gated_deltanet.py ###
        self.mode = config.mode # attn_mode
        ...
        self.conv_zero_init = config.conv_zero_init
        self.conv_size = config.conv_size
        self.conv_bias = config.conv_bias # = False

        # self.head_dim = head_dim # defined
        # self.num_heads = num_heads # defined
        self.num_v_heads = config.num_v_heads if config.num_v_heads is not None else H
        ...
        self.use_qk_l2norm_in_kernel = config.use_qk_l2norm_in_kernel
        self.layer_idx = layer_id

        # Consistency check: Ensure expand_v produces integer values
        if not math.isclose(self.num_v_heads * self.head_dim * self.expand_v, self.value_dim, rel_tol=1e-5):
            raise ValueError(
                f"expand_v={self.expand_v} does not produce an integer value when multiplied by key_dim={self.key_dim}. "
                f"Resulting value_dim would be {self.num_v_heads * self.head_dim * self.expand_v}, which is invalid for nn.Linear.",
            )
...
class TMix_openpangukda(TMix_openpangugdn_base):
    ...
    def _reset_parameters(self):
        print("Called _reset_parameters on TMix_qwen3kda layer ", self.layer_id)
...
class CMix_openpangu(nn.Module):
    ...
    def __init__(self, config, layer_id):
        ...
        self.act_fn = torch.nn.SiLU() #ACT2FN[config.hidden_act]G
...
class PanguEmbeddedDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # debug_tensor(win_input, f"layer_{self.layer_id}.win_input")

        dx, last_chanmix_state = self.mlp(self.post_attention_layernorm(win_input), s)

        # debug_tensor(dx, f"layer_{self.layer_id}.hidden_states_mlp")
...
def ckpt(block:PanguEmbeddedDecoderLayer, *block_args, **block_kwargs):
    if block.training and block.config.train.grad_cp == 1: # and 'fsdp' not in block.config.train.strategy: # FSDP has its own checkpointing wrapper
        ...
        results = torch.utils.checkpoint.checkpoint(block, *block_args, **block_kwargs, use_reentrant=False)
    else:
...
class PanguEmbeddedDecoder(nn.Module):
    ...
    def prepare_shared(self, x):
        ...
        if config.rope is not None and T > shared.angles.size(0):
            ...
            shared.angles = generate_rotary_embedding(max_ctx_len, config.head_size, config.rope.percent, config.rope.base * config.rope.rebase, config.rope.rescale).to(self.norms[0].weight)
        # print (shared.angles)
    ...
    #     self, x,
    #     last_model_state:ModelState|None = None,
    #     past_key_values: Cache | list[torch.FloatTensor] | None = None
    # ):
    #     config : Transformer_Config = self.config.model
    #     B, T, C = x.size()
    #     self.prepare_shared(x)
    ...
    #     return last_model_state, past_key_values
    ...
    def forward(
        ...
        # if any([
        #     'rwkv6' in attention_type,
        #     'gla' in attention_type,
        #     'rwkv7' in attention_type,
        # ]):
        #     last_model_state = self.forward_preamble(x, last_model_state)
        # elif attention_type in ['gdn', 'kda']:
        #     past_key_values = self.forward_preamble_gdnlike(x, past_key_values)


        last_model_state = self.forward_preamble(x, last_model_state)
        use_cache = use_cache if use_cache is not None else (config.use_cache if not self.training else False)
        if use_cache and not isinstance(past_key_values, Cache):
            past_key_values = Cache.from_legacy_cache(past_key_values)
        ...
        x = self.norms[0](x)
        # debug_tensor(x, "last_hidden_state")
...
class Model_openpangu(nn.Module): # Qwen3CausalLM
    ...
    def forward(
        ...
        **kwargs
        ...
        # debug_tensor(results.logits[:,-1:,:], f"logits")
